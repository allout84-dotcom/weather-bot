...
def fetch_schedule_text(target: datetime) -> str:
    """
    민주당 일정 페이지에서 당일 일정만 추출
    페이지 구조:
      정청래 당대표  2026-05-08   ← PERSON_RE 매칭
      ① 08:00 봉사활동 / 장소     ← ITEM_RE 매칭
      ② 09:00 최고위원회의 / 장소
      한병도 원내대표  2026-05-08
      ① 09:00 ...
    """
    ...
    for tag in soup.select("script, style, nav, header, footer, "
                           ".gnb, .lnb, .header-wrap, .footer-wrap"):
        tag.decompose()
...
def check_schedule():
    ...
    if not text:
        print(f"[일정] {date_key} 내용 없음 (미등록 or 파싱 실패)")
        return
